...
% 
% Of the 7 plotted solutions, 6 of them decay while oscilating therefore 100% of
% solutions decay while oscilating. This means that 0% of the solutions
% behave differently. The exception is the constant value solution.
%
% (c) Solve the DE and write the exact solution. Explain why this justifies
% your answer in (b).
% 
% Exact Solution:
% x(t) = exp(-t/2) * ( c1 * cos(2t) - c2 * sin(2t) )
% This solution justifies the answer in b as the exp(-t/2) term approaches
% zero as t approaches infinity, contributing to decay. And the other term
% consists of a cosine and sine term which contribute to oscilation.
% Together, this becomes decay while oscilating.
...
% (a)
%
% <<exercise_2a_image.png>>
% 
% (b)
% 
% % Of the 7 plotted solutions, 6 of them grow, with exception withe the 
% constant solution, therefore 100% of solutions grow. This means that 0% 
% of the solutions behave differently. The sign of the growth depends on 
...
% This justifies the results in (b) as the c2 term goes to zero as t grows,
% and the c1 term governs. This term blows up and its sign is dependent on
% the sing of t, hence suporting (b).
%
...
% (a)
%
% <<exercise_3a_image.png>>
%
% (b)
% 
% % Of the 7 plotted solutions, 6 of them decay, with exception withe the 
% constant solution, therefore 100% of solutions decay. This means that 0% 
% of the solutions behave differently. The sign of the decay depends on 
% the initial conditions.
%
% (c)
% 
% Exact Solution:
% x(t) = c1*exp(t *(sqrt(2)-sqrt(3)) /2) +c2*exp(-t *(sqrt(3) + sqrt(2)) /2)
% This justifies the results in (b) as the both terms goes to zero as t grows,
% since the value of t negated.
%
...
% (b)
% Save the constant solutions, 100% of the solutions will oscilate
% indefinitely. This is due to the c3 and c4 terms that are simple
% trignometric functions. In the case that c3 = c4 = 0. The first term 
% govenr and lead to oscilation with decay due to the negative exponent.   
...
% Calling Solver
[t, y] = DE2_chackoed(p, q, g, t0, tN, y0, y1, h);

plot(t, y);
xlabel('t');
ylabel('y');
legend('Numerical Solution From DE2 chackoed');
title("Exercise 7: Solutions to y'' + exp(-t/5) y' + (1-exp(-t/5)) y = sin(2*t)");

% The two solutions match well. There are no major differences. 